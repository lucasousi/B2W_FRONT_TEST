import './psychicmons-store-home.scss';
...
export const PsychicmonsStoreHome = () => {
  const psychicTypeID = 14;
  const pokemonsPerPage = 12;
  const fakeArrayToLoadingSkeleton = [...Array(pokemonsPerPage).keys()];
  const pageTitle = 'Loja de pokémons do tipo Psíquico';
  const pageSubtitle = 'Aqui você encontra os principais pokémons psíquicos para sua pokedéx.';
  ...
  useEffect(() => {
    ...
    getSummarizedPokemons(psychicTypeID);
        ...
        {isLoadingDetailedPokemons
          ...
          : formattedPokemons?.map((formattedPokemon, index) => (
              <Grid
                ...
                className="home-container__pokecard-container"
              >
                <PokemonCard formattedPokemon={formattedPokemon} buyButtonColor="default" />
              </Grid>
            ))}
      </Grid>
    </section>
...
export default PsychicmonsStoreHome;
